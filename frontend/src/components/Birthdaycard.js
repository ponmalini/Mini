...
function Birthdaycard() {
  ...
  const fetchCards = () => {
    axios
      .get('http://localhost:3500/api/auth/getbirthdayparties')
      .then((response) => {
  ...
  const handleDelete = (id) => {
    axios
      .delete(`http://localhost:3500/api/auth/deletebirthdayparties/${id}`)
      .then(() => {
  ...
  const handleAdd = () => {
    ...
    axios
      .post('http://localhost:3500/api/auth/birthdayparties', {
        title: newTitle,
        url: newImage,
      })
      .then((response) => {
        setCardData((prevData) => [...prevData, response.data.newday]);
        setEditingCard(null);
        setNewTitle('');
        setNewImage('');
  ...
  const handleEdit = () => {
    ...
    axios
      .put(`http://localhost:3500/api/auth/updatebirthdayparties/${editingCard._id}`, {
        title: newTitle,
        url: newImage,
      })
      .then(() => {
