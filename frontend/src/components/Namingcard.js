...
function Namingcard() {
  ...
  const fetchCards = () => {
    axios
      .get('http://localhost:3500/api/auth/getnamingceremony')
      .then((response) => {
  ...
  const handleDelete = (id) => {
    axios
      .delete(`http://localhost:3500/api/auth/deletenamingceremony/${id}`)
      .then(() => {
  ...
  const handleAdd = () => {
    ...
    axios
      .post('http://localhost:3500/api/auth/namingceremony', {
        title: newTitle,
        url: newImage,
      })
      .then((response) => {
        setCardData((prevData) => [...prevData, response.data.newname]);
        setEditingCard(null);
        setNewTitle('');
        setNewImage('');
  ...
  const handleEdit = () => {
    ...
    axios
      .put(`http://localhost:3500/api/auth/updatenamingceremony/${editingCard._id}`, {
        title: newTitle,
        url: newImage,
      })
      .then(() => {
